  ...
  <section class="feature-tag max-w-screen-xl mx-auto mb-8 p-6 bg-gray-200">
    ...
    >
      <div class="md:col-span-2 md:order-last md:pl-14">
        <h2 class="mb-3 md:mb-2 text-3xl md:text-4xl">{{ mainFeature.title }}</h2>
        <p class="mb-3 md:mb-4 text-2xl md:text-4xl">{{ mainFeature.subtitle }}</p>
        <p class="mb-0 md:mb-4 text-sm md:text-lg">{{ mainFeature.description }}</p>
        <NuxtLink
          to="/projects"
          class="
            hidden
...
const moreFeature = 'discover more'
const mainFeature = computed(() => props.features?.[0] ?? {})
const subFeatures = computed(() => props.features?.slice(1) ?? [])
</script>
